...
## Answer generation: Claude or grounded fallback

Embeddings and retrieval are always real. Set an Anthropic key and answers are
generated by **Claude** (official `@anthropic-ai/sdk`, streamed):

```bash
# .env
ANTHROPIC_API_KEY=sk-ant-...
CHAT_MODEL=claude-opus-4-8   # optional; e.g. claude-haiku-4-5 for faster/cheaper support
```

With no key, the app runs a **grounded fallback** that composes the answer
extractively from the retrieved passages — so it always works, and if a live
Claude call fails it degrades gracefully to this instead of erroring. The
provider switch lives in [`src/lib/ai/chat.ts`](src/lib/ai/chat.ts).
